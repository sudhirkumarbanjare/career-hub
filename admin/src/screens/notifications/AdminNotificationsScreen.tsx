...
export interface AdminNotificationsScreenProps {
  onBack: () => void;
  onNavigateAction?: (deepLink?: string) => void;
...
export const AdminNotificationsScreen: React.FC<AdminNotificationsScreenProps> = ({
  ...
  const handleMarkAllRead = () => {
    AdminService.markAllAdminNotificationsAsRead();
    setNotifications([...AdminService.getAdminNotifications()]);
  };

  const handlePressNotification = (notif: AppNotification) => {
    AdminService.markAdminNotificationRead(notif.id);
    setNotifications([...AdminService.getAdminNotifications()]);
    if (notif.deepLink && onNavigateAction) {
      onNavigateAction(notif.deepLink);
    }
  };
        ...
        ListEmptyComponent={
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptyTitle}>
              {filter === 'unread' ? 'No Unread Alerts' : 'No System Alerts'}
            </Text>
            <Text style={styles.emptySub}>
              {filter === 'unread'
                ? 'All pending administrative alerts and approvals have been reviewed.'
                : 'Your platform inbox is completely clear.'}
            </Text>
          </View>
        }
        renderItem={({ item }) => (
          ...
          >
            <Card style={[styles.notifCard, !item.read ? styles.unreadCard : null]}>
              ...
              <View style={styles.cardFooter}>
                {item.deepLink ? (
                  <Text style={styles.tapText}>Review item →</Text>
                ) : (
                  <View />
                )}
                <TouchableOpacity
